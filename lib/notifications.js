...
// Fires from submitRating (see app/dashboard/requests/[id]/actions.js) when
// a client leaves a 5-star rating and a Google review link is configured.
// Email only — a one-off ask, not something that belongs in the ongoing
// notifications list.
export async function notifyClientReviewRequest({ clientEmail, businessName, reviewUrl }) {
  if (!clientEmail || !reviewUrl) return;

  await sendEmail({
    to: clientEmail,
    subject: "Thanks for the 5 stars! One more favor?",
    html: wrapper(
      `<p>Thanks for the great rating${businessName ? `, ${businessName}` : ""} — glad we hit the mark!</p>
       <p>If you have a minute, a public Google review helps other small businesses find us. No pressure either way.</p>`,
      reviewUrl,
...
// Fires when staff clicks "Send check-in" on an inactive subscriber (see
// staff/actions.js's sendCheckIn). Email + in-app, same as most notify
// functions here — unlike the one-off review request, this is meant to
// show up in the client's Notifications tab too as a reason to log back in.
export async function notifyClientCheckIn({ clientId, clientEmail, businessName }) {
  if (clientEmail) {
    await sendEmail({
      to: clientEmail,
      subject: "Just checking in",
      html: wrapper(
        `<p>Hey${businessName ? ` ${businessName}` : ""} — it's been a bit since your last request, so we wanted to
         check in. Anything we can help with, or flyers you'd like to get moving on?</p>`,
        `${PORTAL_URL}/dashboard/new-request`,
        "Submit a request"
      ),
    });
  ...
  if (clientId) {
    await createNotification({
      ...
      body: "Anything we can help with? Submit a request whenever you're ready.",
      link: "/dashboard/new-request",
    });
  }
}
